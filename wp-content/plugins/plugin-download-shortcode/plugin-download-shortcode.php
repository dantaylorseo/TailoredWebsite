<?php
/**
 * Plugin Name: Plugin Download Shortcode
 * Plugin URI: http://www.tailoredinternetmarketing.com/blog/new-twitter-button/
 * Description: A plugin that allows you to display the number of times a plugin has been downloaded. 
 * Version: 1.0
 * Author: Dan Taylor
    ...
    class drt_pds_plugin {
		...
		public function get_plugin_downloads($slug) {
			if ( false === ( $call_api = get_transient( $slug ) ) ) {
			  $call_api =  $this->plugins_api( 'plugin_information', array( 'slug' => $slug ) );
			  set_transient( $slug, $call_api );
			}
